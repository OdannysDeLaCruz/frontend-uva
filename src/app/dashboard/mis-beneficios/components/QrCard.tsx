...
function QrCard({
  ...
  return (
    <div className="bg-white rounded-xl p-2 md:p-6">
      ...
      {qrs.length === 0 ? (
        <p className="text-gray-600 text-sm">No hay beneficios</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {qrs.map((qr) => (
            <button
              key={qr.code}
              onClick={() => onSelect(qr)}
              className="w-full border border-gray-300 rounded-xl p-4 transition text-left cursor-pointer"
            >
              <div className="flex gap-4">
                    ...
                    <div className="mt-2 text-md">
                      {qr.status === "USED" ? (
                        ...
                        <span className="text-amber-600">
                          Expira en{" "}
                          <strong>{getTimeRemaining(qr.expiresAt)}</strong>
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex flex-col items-center min-w-[100px]">
                  <QRCodeCanvas value={qr.qr} size={100} />
                  <span className="mt-2 text-sm text-gray-500">Código</span>
